...
import { analyzeDocument } from "./parser";
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("htmlNestingLinter.showInfo", showInfoPanel)
  );
...
function isSupported(doc: vscode.TextDocument): boolean {
  if (!vscode.workspace.getConfiguration("htmlNestingLinter").get<boolean>("enable", true)) {
    return false;
  }
...
function getSeverity(): vscode.DiagnosticSeverity {
  const cfg = vscode.workspace.getConfiguration("htmlNestingLinter").get<string>("severity", "error");
  switch (cfg) {
...
function validateDocument(doc: vscode.TextDocument) {
  ...
  const errors = analyzeDocument(content, doc.languageId);
...
function buildMessage(parent: string, child: string, reason: string): string {
...
function buildRelatedInfo(
  ...
  range: vscode.Range,
  err: ReturnType<typeof analyzeDocument>[0]
  ...
  if (err.rule.alternatives.length > 0) {
    ...
    info.push(new vscode.DiagnosticRelatedInformation(
...
function getInfoPanelHtml(): string {
