...
import { RULES_MAP, NestingRule } from "./nestingRules";
...
export interface NestingError {
  ...
  /** Tag hijo inválido */
  child: string;
  /** Posición del tag hijo en el texto original */
  offset: number;
  /** Largo del tag de apertura del hijo */
  length: number;
  /** Regla violada */
  rule: NestingRule;
}
 ...
 */
export function extractMarkupRegions(content: string, languageId: string): { text: string; baseOffset: number }[] {
  switch (languageId) {
    case "html":
...
function extractJsxRegions(content: string): { text: string; baseOffset: number }[] {
  const regions: { text: string; baseOffset: number }[] = [];
...
function extractTemplateRegion(content: string): { text: string; baseOffset: number }[] {
...
export function tokenizeTags(text: string): TagToken[] {
  ...
  while ((match = tagRegex.exec(text)) !== null) {
    ...
    let type: "open" | "close" | "self-closing";
    if (isClose) {
      type = "close";
 ...
 */
export function findNestingErrors(tokens: TagToken[]): NestingError[] {
...
export function analyzeDocument(
  content: string,
  languageId: string
): Array<NestingError & { absoluteOffset: number }> {
  const allErrors: Array<NestingError & { absoluteOffset: number }> = [];
